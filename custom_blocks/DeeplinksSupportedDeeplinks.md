---
name: Deeplinks - supported deeplinks
---
# What are the supported deeplinks?

The Purchasely SDK manages deeplinks for:

- **Screens & Paywalls**  
  You can copy the deeplink of a screen by clicking on the 🔗 button of the Screen in the [Paywalls & Screens](https://console.purchasely.io/screen) section  of the Purchasely Console

  [block:image]{"images":[{"image":["https://files.readme.io/6f25cd0-image.png",null,""],"align":"center","border":true}]}[/block]

  They have the shape `app_scheme://ply/presentations/[screen_id]`

<br />

- **Placements**  
  You can copy the deeplink of a <<glossary:placement>> by clicking on the 🔗 button of the <<glossary:placement>> in the [Placements](https://console.purchasely.io/placements) section of the Purchasely Console

  [block:image]{"images":[{"image":["https://files.readme.io/be1c577-image.png",null,""],"align":"center","border":true}]}[/block]

  They have the shape `app_scheme://ply/placements/[placement_id]`

<br />

- **Cancellation survey**  
  This deeplink `app_scheme://ply/cancellation_survey` can be used to display a simple cancellation survey

  [block:image]{"images":[{"image":["https://files.readme.io/7fefc11-image.png",null,""],"align":"center","border":true}]}[/block]

<br />

- **Update billing**  
  This deeplink `app_scheme://ply/update_billing` can be used to open the App Store / Play Store setttings to update credit card after a payment error