...
| Event | Description |
|---|---|
| `SUBSCRIPTION_STARTED` | Sent when the user purchased a product, whether it is the start of a trial or a regular purchase of a consumable product |
...
| `TRIAL_NOT_CONVERTED` | Sent when a user finishes their trial period without renewing to a paid-period |
...
You can also override an event's token for each store, individually, provided you have created a store-specific token for that event in the Adjust Console.
