...
dependencies {
    ...
    implementation("io.purchasely:player:6.0.0-rc.3") // optional, video support
}
```

Build requirements: **Gradle 9.3.0+** (built with 9.6.1), **Kotlin 2.2.x** min (built with 2.3.x, K2 compiler), JDK 11, `minSdk 23`, `compileSdk 36`.

> 📘 Target JVM 11
>
> The reified Kotlin entry points (`interceptAction<T> { … }`, `removeActionInterceptor<T>()`) are `inline` functions targeting JVM 11. <br />Compile your Kotlin module with `jvmTarget = 11`, or use the non‑inline `Class`‑based overload.
