...
metadata:
  ...
  description: >-
    Learn how Purchasely helps you measure leading KPIs directly in the Console
    and export experiment exposure data to your analytics stack for full
...
[Audiences](audiences) are user segments letting you to target users with specific In-App Experiences. There are 2 types of Audiences:

1. **Rule-based Audiences** are defined by combining user attributes (built-in or custom) with boolean operators. 
2. **Custom Audiences** are lists of users that can be synchronized from your Data Platform or CRM platform through an API.

Once defined, Audiences can be mapped to specific Screens or Flows within a <Glossary>Placement</Glossary>, enabling hyper-personalization of the In-App Experience.
...
<Image align="center" src="https://files.readme.io/25ffabf6b79c4016567cba256de1343610ea780f04dcf6e0cf10379329f5b519-flows.gif" />
...
* **EXPERIMENT_ASSIGNED** captures the moment of randomization. Use it when you want to analyze all users who were _intended_ to see an experience, regardless of whether they actually saw it. This avoids selection bias.
* **EXPERIMENT_VIEWED** captures actual exposure. Use it when you want to analyze only users who were _actually exposed_ to the experience.

## Data Export Channels

These events (along with all [UI/SDK events](ui-sdk-events-list) and Custom Events) can reach your data platform through four channels:

### 1. UI / SDK event listener

This [Event Listener / delegate](ui-sdk-events#how-to-leverage-them-by-implementing-an-event-delegate-inside-the-app) lets you intercept UI / SDK events when their are emitted by the SDK and to forward them directly from the app to your desired analytics / CRM platform.
