...
If your app takes over the purchase flow (**Observer** mode or a custom `purchase` action interceptor), you need to know which billing plan the offering is selling to pass the matching purchase option to StoreKit. The [action interceptor](action-interceptor) gives you all the commitment information:
