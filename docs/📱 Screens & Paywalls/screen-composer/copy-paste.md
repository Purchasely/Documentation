---
title: Copy & Paste components
excerpt: >-
  This article explains about how can you copy and paste components in the
  Composer
deprecated: false
hidden: false
metadata:
  title: ''
  description: ''
  robots: index
next:
  description: ''
---
Purchasely lets you copy and paste customized components across multiple screens using the Screen Composer.

You can copy and paste a single component or an entire list of components from one screen to another.

This approach:  
✔ Saves time for designers.  
✔ Reduces the chances of errors.  
✔ Enhances efficient scalability.

[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/ac9b0c5bfb57d3524281679f81340521e4c2f39a238bc743d39055e161ad3605-copy_paste.gif",
        "",
        ""
      ],
      "align": "center",
      "border": true
    }
  ]
}
[/block]