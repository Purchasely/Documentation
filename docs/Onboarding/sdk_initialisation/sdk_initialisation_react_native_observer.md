...
> 📘 In v6 the SDK is started with the fluent `Purchasely.builder(...)`. The old `Purchasely.start({...})` object method (and its `runningMode` / `logLevel` enums and the `storeKit1: true/false` boolean) has been removed. The default running mode is now `'observer'`, so observer mode no longer needs to be requested explicitly — but passing `.runningMode('observer')` makes the intent clear. In observer mode your existing in-app purchase infrastructure keeps control of the purchase flow.
