...
| **Light line**                | Trend, showing the overall direction of the balance over the selected time period |
...
**New** counts all paid subscriptions that started during the period. This includes first-time purchases, free trials conversions, reactivations of previously expired subscriptions, and subscriptions resuming after a billing retry recovery. Users switching from one paid plan to another do not appear in this chart as they were already on a paid plan before.

**Churn** counts all paid subscriptions that terminated during the period. This includes voluntary cancellations that reached their expiry date, involuntary churn from failed grace periods, and refunds. Users switching from one plan to another, do not appear here as a churn event if their new plan is still a paid plan.
...
**Average daily balance** divides each period's balance by the number of periods it covers. This smooths out spikes caused by weekends, holidays, or one-off events.

**Trend** fits a line through the balance values to reveal the underlying direction for the selected time period. If the trend line slopes upward, your subscriber base is growing on average. If it slopes downward, churn is outpacing acquisition.
...
For **deferred plan changes** (primarily Google Play), both the +1 churn and +1 new appear on the date the downgrade is _initiated_, even though the user continues using the old plan until the next renewal date. This can temporarily overstate both New and Churn volumes before the actual plan switch takes effect.
...
### How is the average balance different from the balance?
...
### Does the chart include subscriptions in grace period and billing retry?
...
Summing daily New and Churn values across a month may not exactly match the monthly totals. This is because a subscription that churns and reactivates within the same month appears as both a Churn and a New event in daily view, but the net effect may be collapsed in monthly view as churn are only counted at the end of the period. This is standard behavior and consistent with how the Subscription Retention chart handles granularity differences.
