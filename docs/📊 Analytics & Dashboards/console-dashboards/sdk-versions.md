...
This is critical for planning SDK deprecation, validating feature rollouts, and ensuring your users have access to the latest paywall designs and capabilities. Since the Purchasely SDK version determines which features are available to each user (e.g., SDK 5.0.0+ is required for Screen Composer), tracking version distribution helps you make informed decisions about when to drop support for older versions.
...
Use the date range picker to select the time window you want to analyze. Adjust this to focus on a specific app release period or to observe long-term migration trends.

## Granularity

The chart defaults to a **daily** view, with one bar per day. This provides the most detailed picture of how SDK version adoption evolves.
...
- **Ensure Screen Composer compatibility** -- SDK 5.0.0+ is required for Screen Composer paywalls. Use the chart to verify what share of your users can see your latest paywall designs. Users on older versions will fall back to legacy screens.
