...
| `SUBSCRIPTION_REFUND_REVERSED` | The App Store reversed a previously granted refund following a dispute raised by the customer. The subscription is active again on its original period, with an unchanged renewal date, and the subscriber must get the subscription benefits back.<br><br>iOS only | 1. Restore the premium benefits you removed on `SUBSCRIPTION_REFUNDED_REVOKED` |
| `ONE_TIME_PURCHASE_REFUND_REVERSED` | The App Store reversed a previously granted refund of a one-time purchase following a dispute raised by the customer. The purchase is valid again and the buyer must get its content or benefits back.<br><br>iOS only | 1. Restore the content or benefits you removed when the purchase was refunded |
